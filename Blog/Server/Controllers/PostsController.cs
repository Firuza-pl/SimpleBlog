...
namespace Blog.Server.Controllers
{
    ...
    {
        //ISender is part of Mediatr that is used to handle sending request (command and queries)
        //it allows us to send request to the appropriate handler without know the spesific handler class.
        //it achives loose coupling


        //IMediatr is another part of Mediatr library. combines the functionality of ISender and IRequestHandler
        //it proiviode cnetral point of dispatching requetst and executing assocoiated handler
       
        //we commonly use ISender instead of IMeadiatr, because sender show specific interface for sending requst
        //while IMEadiatr is more general interface to combine both sending  and handling request
        ...
        {
            var postsDTO = await _mediator.Send(new GetPaginatedPostsQuery
                (paginationParams.Page, paginationParams.ItemsCountPerPage, searchParams.SearchTerm));
